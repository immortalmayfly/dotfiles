...
# shell parsing stuff
_fzf_tab_awk="$(command -v gawk 2>/dev/null || echo awk)"
_fzf_tab_sed="$(command -v gsed 2>/dev/null || echo sed)"
_fzf_tab_grep="$(command -v ggrep 2>/dev/null || echo grep)"

_fzf_tab_awk_escape() {
    "$_fzf_tab_sed" 's/\\/\\\\\\\\/g; s/[[*^$.]/\\\\&/g' <<<"$1"
}

_fzf_tab_shell_split() {
    "$_fzf_tab_grep" -E -o \
        -e '\|+|&+|<+|>+' \
        -e '[;(){}&\|]' \
...
_fzf_tab_unbuffered_awk() {
    # need to get awk to be unbuffered either by using -W interactive or system("")
    "$_fzf_tab_awk" -W interactive "${@:3}" "$1 { $2; print \$0; system(\"\") }" 2>/dev/null
}

_fzf_tab_flatten_subshells() {
    (
...
_fzf_tab_find_matching_bracket() {
    local count=0
...
_fzf_tab_parse_dq() {
    ...
    if [[ "$last" == \"* ]]; then
        ...
        while true; do
            ...
            shell_start="$(<<<"$line" "$_fzf_tab_grep" -E -o '^(\\.|\$[^(]|[^$])*\$\(')"
            string_end="$(<<<"$line" "$_fzf_tab_grep" -E -o '^(\\.|[^"])*"')"
            ...
            if (( ${#string_end} && ( ! ${#shell_start} || ${#string_end} < ${#shell_start} )  )); then
                ...
                split="$(<<<"$line" _fzf_tab_shell_split)"
                if ! split="$(_fzf_tab_parse_dq <<<"$split")"; then
                    # bubble up
                    printf '%s\n' "$split"
                    return 1
                fi
                if ! num="$(_fzf_tab_find_matching_bracket ')' <<<"$split")"; then
...
_fzf_tab_unquote_strings() {
    ...
    while IFS= read -r line; do
        if [[ "$line" =~ ^\'[^\']*\'?$ ]]; then
            ...
            "$_fzf_tab_sed" -r 's/\\(.)/\1/g' <<<"${line:1-1}"
        elif [[ "$line" == *\\* && ! "$line" =~ [\$\"\'\`\<\>\|\;] ]]; then
            # all special characters are quoted with backslashes
            "$_fzf_tab_sed" -r 's/\\(.)/\1/g' <<<"$line"
        else
...
_fzf_tab_compspec() {
    if [[ "$2" =~ .*\$(\{?)([A-Za-z0-9_]*)$ ]]; then
        printf '%s\n' 'complete -F _fzf_tab_complete_variables'
       ...
       # If the command word is a full pathname, a compspec for the full pathname is searched for first.  If no compspec is found for the full pathname, an attempt is made to find a compspec for the portion following the final slash.  If those searches do not result in a compspec, any compspec defined with the -D option to complete is used as the default
        complete -p -- "$1" || complete -p -- "${1##*/}" || complete -p -D || printf '%s\n' 'complete -o filenames -F _fzf_tab_fallback_completer'
    fi
}

_fzf_tab_fallback_completer() {
...
_fzf_tab_complete_commands() {
    # commands
    compopt -o filenames
    readarray -t COMPREPLY < <(compgen -abc -- "$2")
}

_fzf_tab_complete_variables() {
    if [[ "$2" =~ .*\$(\{?)([A-Za-z0-9_]*)$ ]]; then
...
fzf_tab() {
    ...
    if [[ "$line" =~ [^[:space:]] ]]; then
        readarray -t raw_comp_words < <(_fzf_tab_parse_line <<<"$line")
    fi

    if [[ ${#raw_comp_words[@]} -gt 1 ]]; then
        _fzf_tab_expand_alias "${raw_comp_words[@]}"
    fi
    readarray -t COMP_WORDS < <(printf '%s\n' "${raw_comp_words[@]}" | _fzf_tab_unquote_strings)
...
fzf_bash_completer() {
    ...
    local compspec
    if ! compspec="$(_fzf_tab_compspec "$@" 2>/dev/null)"; then
        return
    fi
    ...
    (
        ...
        coproc (
            ...
            ) | $_fzf_tab_sed -un "/$_fzf_sentinel1$_fzf_sentinel2/q; p" \
              | _fzf_tab_auto_common_prefix "$raw_cur" \
              | _fzf_tab_unbuffered_awk '$0!="" && !x[$0]++' '$0 = $0 sep "\x1b[37m" substr($0, 1, len) "\x1b[0m" sep substr($0, len+1)' -vlen="${#raw_cur}" -vsep="$_FZF_COMPLETION_SEP"
        )
        local coproc_pid="$COPROC_PID"
        value="$(_fzf_tab_selector "$1" "$raw_cur" "$3" <&"${COPROC[0]}")"
        code="$?"
        value="$(<<<"$value" tr \\n \ )"
        value="${value% }"
        ...
        descend_process () {
            printf '%s\n' "$1"
            for pid in $(ps -ef | "$_fzf_tab_awk" -v ppid="$1" '$3 == ppid { print $2 }'); do
                descend_process "$pid"
            done
        }
...
_fzf_tab_complete() {
    ...
    while (( $# > 1 )); do
        ...
        -X)
            ...
            shift ;;
        -o)
            _fzf_tab_compopt -o "$2"
            shift ;;
        -A)
            local compgen_opts+=( "$1" "$2" )
            shift ;;
        -P)
            local compl_prefix="$(_fzf_tab_awk_escape "$2")"
            shift ;;
        -S)
            local compl_suffix="$(_fzf_tab_awk_escape "$2")"
            shift ;;
        -[a-z])
            compgen_actions+=( "$1" )
            ;;
    ...
    if [ -n "$compl_function" ]; then
        ...
        if [ "$?" = 124 ]; then
            local newcompspec
            if ! newcompspec="$(_fzf_tab_compspec "$@" 2>/dev/null)"; then
                return
    ...
    if [[ "$compl_filenames" == 1 ]]; then
        local dir_marker=_fzf_tab_dir_marker
    else
    ...
    (
        (
            ...
            printf '\n'
        ) | _fzf_tab_apply_xfilter "$compl_xfilter" \
          | _fzf_tab_unbuffered_awk '$0!=""' 'sub(find, replace)' -vfind='.*' -vreplace="$(printf %s "$compl_prefix" | "$_fzf_tab_sed" 's/[&\]/\\&/g')&$(printf %s "$compl_suffix" | "$_fzf_tab_sed" 's/[&\]/\\&/g')" \
          | if IFS= read -r line || (( ${#COMPREPLY[@]} )); then
              ([[ -z "$line" ]] || printf '%s\n' "$line"; cat) | _fzf_tab_quote_filenames "$@"
            else
                # got no results
                local compgen_opts=()
                ...
                if [ -n "${compgen_opts[*]}" ]; then
                    ...
                    | compl_filenames=1 _fzf_tab_quote_filenames "$@" \
                    | _fzf_tab_dir_marker
                fi
            fi
...
_fzf_tab_apply_xfilter() {
    ...
    pattern="$("$_fzf_tab_sed" 's/\(\(^\|[^\]\)\(\\\\\)*\)&/\1'"$word"'/g' <<<"${1:1}")"
...
_fzf_tab_dir_marker() {
    ...
    while IFS= read -r line; do
        ...
        if [[ "$compl_noquote" != 1 && "$expanded" == *\\* ]]; then
            expanded="$("$_fzf_tab_sed" -r 's/\\(.)/\1/g' <<<"$expanded")"
        fi
